...
export function HomePage({ onNavigate }: HomePageProps) {
  ...
  const fetchHomePageData = async () => {
    try {
      ...
      // SIMPLIFIED: Just fetch opportunities to test connection
      const opportunities = await apiClient.getOpportunities();
      console.log('Opportunities from backend:', opportunities);
      
      // Use fallback data for now
      setPlatformStats({
        total_volunteers: 12500,
        total_ngos: 550,
        total_hours: 52000,
        completed_tasks_today: 245
      });
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">What is Micro-Volunteering?</h2>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            {ngoPartners.map((ngo, index) => (
              ...
              >
                <div className="text-gray-700 font-medium">{ngo}</div>
              </div>
            ))}
          </div>
        </div>
